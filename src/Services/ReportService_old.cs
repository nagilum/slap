...
public class ReportService_old : IReportService
{
    ...
    private void AddQueueEntriesToHtmlReport(ref string html, UrlType urlType)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            // URL.
            sb.Append($"<tr><td class=\"url{(entry.UrlType == UrlType.InternalPage ? "-875" : "-700")}\"><a href=\"{entry.Url}\">{entry.Url}</a></td>");
            
            // Accessibility issues.
            if (entry.UrlType == UrlType.InternalPage)
            {
    ...
    private void AddStatsToHtmlReport(ref string html)
    {
        ...
        foreach (var urlType in Enum.GetValues<UrlType>())
        {
            ...
            {
                UrlType.InternalPage => "type-internal-pages.html",
                UrlType.InternalAsset => "type-internal-assets.html",
                UrlType.ExternalPage => "type-external-pages.html",
    ...
    private async Task GenerateOverviewReport()
    {
        ...
        this.AddQueueEntriesToHtmlReport(ref html, UrlType.InternalPage);
        this.AddQueueEntriesToHtmlReport(ref html, UrlType.InternalAsset);
        this.AddQueueEntriesToHtmlReport(ref html, UrlType.ExternalPage);
